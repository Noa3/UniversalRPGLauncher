...
7. **Completed K-018 (MZ inventory)**: extended the same result with optional-section counts (Classes/Skills/Items/Weapons/Armors/Enemies/Troops), System.json `switches`/`variables` name-array counts, and physical `Map###.json` file count (cap 1000). Absent sections stay silent; malformed ones get per-file diagnostics. New tests: `TestMzDataDirectory` now 7 tests. Suite total **205/205 green**. Note: malformed-JSON fixtures log Godot engine-side "Parse JSON failed" lines by design of `Json.ParseString`; behavior is deterministic and asserted via diagnostics.
