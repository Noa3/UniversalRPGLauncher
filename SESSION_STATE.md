...
|K-032 remains IN PROGRESS; K-033 through K-039 and K-042/K-043 are DONE — LMU page command vectors are now decoded with bounded limits, malformed vectors are rejected, and decoded native commands are converted into `Rm2kMap.EventCommand` instances for the deterministic scheduler. Foreign JavaScript/Ruby/native plugins remain non-executable; compatible behavior must use native/whitelisted reimplementation.|
