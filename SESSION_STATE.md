...
|K-032 integration is DONE for the interpreter handoff — bounded choices and numeric input now pause execution, resume only after explicit UI submission, and write resolved input to simulation variables. Next: wire presentation state into Godot UI/runtime entry point. Keep MZ detection-only.|
