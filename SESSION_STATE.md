...
|K-032 remains IN PROGRESS; K-033 through K-039 and K-042/K-045 are DONE — official LMU page condition fields now decode switch/variable/item/actor/timer flags and IDs; official page metadata IDs and command-list ID are supported with legacy fallback. Conditions are exposed to the native deterministic selector. Foreign JavaScript/Ruby/native plugins remain non-executable; compatible behavior must use native/whitelisted reimplementation.|
