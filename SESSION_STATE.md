...
|K-032 remains IN PROGRESS; K-033 through K-039 and K-042/K-044 are DONE — decoded native RM2K commands now reach the deterministic scheduler; Enter/Space dispatches an Action event in the facing tile, and successful player movement dispatches a Touch event on the destination tile. Full UI/runtime smoke coverage remains bounded by available imported-game fixtures. Foreign JavaScript/Ruby/native plugins remain non-executable; compatible behavior must use native/whitelisted reimplementation.|
