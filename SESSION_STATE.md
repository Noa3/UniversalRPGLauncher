...
|K-032 runtime/UI wiring is IN PROGRESS — RuntimeLauncher exposes active runtime/update/stop, Rm2kEngineRuntime owns simulation and presentation state, and Main processes the active runtime each frame. Visible map/presentation composition remains next; keep MZ detection-only.|
