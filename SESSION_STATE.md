...
|K-032 remains IN PROGRESS; K-033 through K-039 are DONE — Main renders parsed RM2K lower/upper tile layers, map events, and the simulated player position; forwards WASD/arrow input; supports keyboard and clickable message dismissal, choice selection, and bounded numeric input; avoids per-frame choice-node churn; and exposes an explicit stop control that clears stale presentation UI. Full runtime event execution remains next; keep MZ detection-only.|
