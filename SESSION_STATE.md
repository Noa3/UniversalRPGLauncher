...
|K-032 remains IN PROGRESS; K-033 through K-039 and K-042 are DONE — RM2K now has bounded event-page selection (highest eligible page wins), switch/variable conditions, and a deterministic scheduler for autorun/parallel/action/touch triggers. The native runtime advances the scheduler on virtual-clock ticks. The current LMU parser exposes page metadata but not yet decoded command lists, so real imported map-event execution remains limited until command-list decoding is added. Foreign JavaScript/Ruby/native plugins remain non-executable; compatible behavior must use native/whitelisted reimplementation.|
