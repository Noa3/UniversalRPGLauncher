...
|K-032 remains IN PROGRESS; K-033 through K-038 are DONE — Main renders parsed RM2K lower/upper tile layers, map events, and the simulated player position; forwards WASD/arrow input; supports keyboard and clickable message dismissal, choice selection, and bounded numeric input. Choice controls are rebuilt only when their options/selection change, avoiding per-frame node churn. Full runtime event execution remains next; keep MZ detection-only.|
