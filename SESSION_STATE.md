...
|K-032 remains IN PROGRESS; K-033, K-034, K-035, and K-036 are DONE — Main renders parsed RM2K lower/upper tile layers, map events, and the simulated player position in a bounded preview; forwards WASD/arrow input to simulation movement; dismisses messages; navigates choices; and collects bounded numeric input while presentation waits block movement. The runtime now advances simulation frame count from the deterministic virtual clock. Full runtime event execution remains next; keep MZ detection-only.|
