...
|K-032 remains IN PROGRESS; K-033 through K-037 are DONE — Main renders parsed RM2K lower/upper tile layers, map events, and the simulated player position; forwards WASD/arrow input; supports keyboard and clickable message dismissal, choice selection, and bounded numeric input. The runtime advances simulation frame count from the deterministic virtual clock. Full runtime event execution remains next; keep MZ detection-only.|
