...
|K-032 remains IN PROGRESS; K-033 is DONE — Main renders parsed RM2K lower/upper tile layers, map events, and the simulated player position in a bounded preview. The preview accepts packed and ordinary Godot arrays. Choice/input controls and full runtime event execution remain next; keep MZ detection-only.|
